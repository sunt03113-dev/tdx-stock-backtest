# -*- coding: utf-8 -*-
"""
规则0913B 筛选 —— 20cm（科创板 688 + 创业板 300/301）
...
形态：D-5 放量峰 → D-4 缩量 → D-3 再度放量且股价最高（双峰中间夹一日缩量），
      D-2/D-1 连续缩量，D-0 放量封板。

输出（16 列）：
  D-0 日期
  D-5 单日振幅 / D-3 单日振幅（数值不带 %，前收分别为 D-6 收、D-4 收）
  D-5~D-3 区间涨幅、区间振幅（数值带 %；基收 D-6 收，末收 D-3 收）
  D-2~D-1 区间涨幅、区间振幅（数值带 %；基收 D-3 收，末收 D-1 收）
  D-0 单日振幅（数值不带 %，前收 D-1 收）
  D-0/D-3 成交额百分比（数值带 %）
  T+0~T+6 价格走势（基准价 = D-0 收盘价）

数据源：本地通达信 .day 二进制；涨停判定统一委托 backtest_common.compute_limit_flags（严格 ==）。
"""
import sys
import time
import argparse
...
RULE_NAME = "0913B"
OUTPUT_DIR = Path(r"D:\09work\0913B_backtest")
...
def screen_one(code, day_file, start_int, end_int):
    ...
    for i in idxs:
        ...
        #   D-5~D-3 区间基收 = D-6 收 closes[i-6]；末收 = D-3 收 closes[i-3]
        ...
        d1_close = closes[i - 1]
        d0_close = closes[i]
        if d6_close <= 0 or d4_close <= 0 or d3_close <= 0 or d1_close <= 0 or d0_close <= 0:
            continue
        ...
        # D-5~D-3 区间（i-5, i-4, i-3；slice[i-5:i-2]）
        r53_highs = highs[i - 5: i - 2]
        r53_lows = lows[i - 5: i - 2]
        range53_amp = (r53_highs.max() - r53_lows.min()) / d6_close * 100.0
        range53_gain = (d3_close - d6_close) / d6_close * 100.0

        # D-2~D-1 区间（i-2, i-1；slice[i-2:i]）
        r21_highs = highs[i - 2: i]
        r21_lows = lows[i - 2: i]
        range21_amp = (r21_highs.max() - r21_lows.min()) / d3_close * 100.0
        range21_gain = (d1_close - d3_close) / d3_close * 100.0
        ...
        rows.append({
            ...
            "D-5~D-3区间涨幅": f"{round(float(range53_gain), 2)}%",
            "D-5~D-3区间振幅": f"{round(float(range53_amp), 2)}%",
            "D-2~D-1区间涨幅": f"{round(float(range21_gain), 2)}%",
            "D-2~D-1区间振幅": f"{round(float(range21_amp), 2)}%",
