# -*- coding: utf-8 -*-
"""
规则0915B 筛选 —— 20cm（科创板 688 + 创业板 300/301）

时间线（D-0 为基准日 i，向前回看历史）：
  D-7 (i-7) : 非涨停；成交额非 20 日最大
  D-6 (i-6) : 非涨停；成交额非 20 日最大
  D-5 (i-5) : 非涨停；成交额为 20 日最大
  D-4 (i-4) : 非涨停；成交额为 20 日最大；最高价为 20 日最高
  D-3 (i-3) : 非涨停；成交额非 20 日最大
  D-2 (i-2) : 非涨停；成交额非 20 日最大
  D-1 (i-1) : 非涨停；成交额非 20 日最大
  D-0 (i)   : 涨停（严格封死 ==）；成交额 20 日最大；最高价 20 日最高（基准日）

形态：D-5/D-4 连续两日成交额峰值（D-4 同时股价最高），前后各两日缩量，D-0 放量封板。

输出（16 列）：
  D-0 日期
  D-5 单日振幅 / D-4 单日振幅（数值不带 %，前收分别为 D-6 收、D-5 收）
  D-5~D-4 区间涨幅、区间振幅（数值带 %；基收 D-6 收，末收 D-4 收）
  D-3~D-1 区间涨幅、区间振幅（数值带 %；基收 D-4 收，末收 D-1 收）
...
RULE_NAME = "0915B"
OUTPUT_DIR = Path(r"D:\09work\0915B_backtest")

OUTPUT_COLUMNS = [
    "股票代码", "股票名称", "D-0日期",
    "D-5振幅", "D-4振幅",
    "D-5~D-4区间涨幅", "D-5~D-4区间振幅",
    "D-3~D-1区间涨幅", "D-3~D-1区间振幅",
    "D-0振幅",
    "D-0/D-4成交额百分比",
...
def screen_one(code, day_file, start_int, end_int):
    ...
    d4_ok[4:] = not_limit[:-4] & is_amtmax[:-4] & is_highmax[:-4]
    # D-5：非涨停 + 成交额最大（无股价最高）
    ...
    for i in idxs:
        ...
        #   D-5~D-4 区间基收 = D-6 收 closes[i-6]；末收 = D-4 收 closes[i-4]
        #   D-3~D-1 区间基收 = D-4 收 closes[i-4]；末收 = D-1 收 closes[i-1]
        ...
        d1_close = closes[i - 1]
        d0_close = closes[i]
        if d6_close <= 0 or d5_close <= 0 or d4_close <= 0 or d1_close <= 0 or d0_close <= 0:
            continue
        ...
        # D-5~D-4 区间（i-5, i-4；slice[i-5:i-3]）
        r54_highs = highs[i - 5: i - 3]
        r54_lows = lows[i - 5: i - 3]
        range54_amp = (r54_highs.max() - r54_lows.min()) / d6_close * 100.0
        range54_gain = (d4_close - d6_close) / d6_close * 100.0

        # D-3~D-1 区间（i-3, i-2, i-1；slice[i-3:i]）
        r31_highs = highs[i - 3: i]
        r31_lows = lows[i - 3: i]
        range31_amp = (r31_highs.max() - r31_lows.min()) / d4_close * 100.0
        range31_gain = (d1_close - d4_close) / d4_close * 100.0
        ...
        # D-0/D-4 成交额百分比
        amt_d4 = amounts[i - 4]
        amt_pct = (amounts[i] / amt_d4 * 100.0) if amt_d4 > 0 else 0.0
        ...
        rows.append({
            ...
            "D-5~D-4区间涨幅": f"{round(float(range54_gain), 2)}%",
            "D-5~D-4区间振幅": f"{round(float(range54_amp), 2)}%",
            "D-3~D-1区间涨幅": f"{round(float(range31_gain), 2)}%",
            "D-3~D-1区间振幅": f"{round(float(range31_amp), 2)}%",
            "D-0振幅": round(float(amp_d0), 2),
            "D-0/D-4成交额百分比": f"{round(float(amt_pct), 2)}%",
