# -*- coding: utf-8 -*-
"""
规则0917E 筛选 —— 20cm（科创板 688 + 创业板 300/301）
...
时间线（D-0 为基准日 i，向前回看历史）：
  ...
  D-2 (i-2) : 涨停（严格封死 ==）；成交额 20 日最大；最高价 20 日最高
  D-1 (i-1) : 非涨停；成交额为 20 日最大
  D-0 (i)   : 非涨停；成交额 20 日最大；最高价 20 日最高（基准日）

形态：D-2 放量封板（成交额最大、股价最高），D-3~D-7 连续缩量非涨停，D-1 再次放量但未涨停，
      D-0 继续放量股价最高但未涨停（基准日）。与 0917D 差异：D-1 成交额最大。

输出（16 列）：
  D-0 日期
  D-2 单日振幅（数值不带 %，前收 D-3 收）
...
RULE_NAME = "0917E"
OUTPUT_DIR = Path(r"D:\09work\0917E_backtest")
...
def screen_one(code, day_file, start_int, end_int):
    ...
    # —— 候选掩码（对齐 D-0=i）——
    # D-0：非涨停 + 成交额最大 + 股价最高（基准日）
    d0_ok = not_limit & is_amtmax & is_highmax
    # D-1：非涨停 + 成交额最大（与 0917D 的差异：这里是 is_amtmax，0917D 是 ~is_amtmax）
