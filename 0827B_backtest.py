# -*- coding: utf-8 -*-
"""
规则0827B 筛选 —— 20cm（科创板 688 + 创业板 300/301）
...
时间线（D-0 为基准日 i，向前回看历史）：
  ...
  D-2 (i-2) : 非涨停；成交额为 20 日最大
  D-1 (i-1) : 非涨停；成交额非 20 日最大
  D-0 (i)   : 涨停；成交额为 20 日最大；最高价为 20 日最高（基准日）

输出：
  1. D-0 日期
  2. D-4 单日振幅（数值不带 %，如 12.6）
  3. D-2 单日振幅（数值不带 %）
  4. D-4~D-2 区间涨幅（数值带 %，如 14.52%）
  5. D-4~D-2 区间振幅（数值带 %）
  6. D-1 单日涨幅（数值不带 %）
  7. D-1 单日振幅（数值不带 %）
  8. D-0 单日振幅（数值不带 %）
  9. D-0/D-2 成交额百分比（数值带 %）
...
RULE_NAME = "0827B"
OUTPUT_DIR = Path(r"D:\09work\0827B_backtest")
...
def screen_one(code, day_file, start_int, end_int):
    ...
    # 有效 i 范围：i-6>=19（D-6 的 20 日窗口起点），即 i>=25
    ...
    for i in idxs:
        ...
        #   D-4 前收 = D-5 收盘 = closes[i-5]
        #   D-2 前收 = D-3 收盘 = closes[i-3]（注意 D-3 与 D-2 相邻，D-2 前收 = D-3）
        #   D-1 前收 = D-2 收盘 = closes[i-2]
        #   D-0 前收 = D-1 收盘 = closes[i-1]
        d5_close = closes[i - 5]   # D-5 收盘 = D-4 前收
        d3_close = closes[i - 3]   # D-3 收盘 = D-2 前收
        d2_close = closes[i - 2]   # D-2 收盘 = D-1 前收
        d1_close = closes[i - 1]   # D-1 收盘 = D-0 前收
        d0_close = closes[i]
        # 防御性除零
        ...
        # D-4 振幅：前收 = D-5 收盘 = closes[i-5]
        amp_d4 = (highs[i - 4] - lows[i - 4]) / d5_close * 100.0
        # D-2 振幅：前收 = D-3 收盘 = closes[i-3]
        amp_d2 = (highs[i - 2] - lows[i - 2]) / d3_close * 100.0
        # D-1 振幅：前收 = D-2 收盘 = closes[i-2]
        amp_d1 = (highs[i - 1] - lows[i - 1]) / d2_close * 100.0
        # D-0 振幅：前收 = D-1 收盘 = closes[i-1]
        amp_d0 = (highs[i] - lows[i]) / d1_close * 100.0

        # D-4~D-2 区间：[i-4, i-2]，区间前一日 = D-5 = closes[i-5]
        rng_highs = highs[i - 4 : i - 1]   # D-4, D-3, D-2 三日的最高价（i-4, i-3, i-2）
        rng_lows = lows[i - 4 : i - 1]     # D-4, D-3, D-2 三日的最低价
        range_amp = (rng_highs.max() - rng_lows.min()) / d5_close * 100.0
        # 区间涨幅：(末收 - 基收)/基收；末收 = D-2 收盘 = closes[i-2]，基收 = D-5 收盘 = closes[i-5]
