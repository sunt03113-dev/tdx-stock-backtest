# -*- coding: utf-8 -*-
"""
规则0908 筛选 —— 20cm（科创板 688 + 创业板 300/301）

时间线（D-0 为基准日 i，向前回看历史）：
  D-7 (i-7) : 非涨停；成交额非 20 日最大
  D-6 (i-6) : 非涨停；成交额非 20 日最大
  D-5 (i-5) : 非涨停；成交额为 20 日最大；最高价为 20 日最高
...
输出：
  1. D-0 日期
  2. D-5 单日涨幅（数值不带 %，如 12.6）
  3. D-5 单日振幅（数值不带 %）
  4. D-4~D-1 区间涨幅（数值带 %，如 14.52%）
  5. D-4~D-1 区间振幅（数值带 %）
  6. D-0 单日振幅（数值不带 %）
  7. D-0/D-5 成交额百分比（数值带 %）
  8. T+0~T+6 价格走势（基准价 = D-0 收盘价）

注意：单日涨幅/振幅数值不带 %；区间涨幅/振幅与成交额百分比数值带 %。
...
RULE_NAME = "0908"
OUTPUT_DIR = Path(r"D:\09work\0908_backtest")
...
def screen_one(code, day_file, start_int, end_int):
    ...
    # D-5(i-5): 非涨停 + 成交额 20 日最大 + 最高价 20 日最高
    d5_ok = np.zeros(n, dtype=bool)
    d5_ok[5:] = not_limit[:-5] & is_amtmax[:-5] & is_highmax[:-5]
    # D-6(i-6): 非涨停 + 成交额非 20 日最大
    ...
    cand = d0_ok & d1_ok & d2_ok & d3_ok & d4_ok & d5_ok & d6_ok & d7_ok
    # 有效 i 范围：i-7>=19（D-7 的 20 日窗口起点），即 i>=26
    ...
    for i in idxs:
        ...
        #   D-5 前收 = D-6 收盘 = closes[i-6]
        #   D-4~D-1 区间前一日 = D-5 收盘 = closes[i-5]
        #   D-0 前收 = D-1 收盘 = closes[i-1]
        d6_close = closes[i - 6]   # D-6 收盘 = D-5 前收
        d5_close = closes[i - 5]   # D-5 收盘 = D-4~D-1 区间基收
        d1_close = closes[i - 1]   # D-1 收盘 = D-0 前收 / 区间末收
        d0_close = closes[i]
        # 防御性除零
        if d6_close <= 0 or d5_close <= 0 or d1_close <= 0 or d0_close <= 0:
            continue

        # 单日涨幅 = (今收 - 前收) / 前收 * 100（输出时不带 %）
        # D-5 涨幅：今收 = D-5 收盘 = closes[i-5]，前收 = D-6 收盘 = closes[i-6]
        gain_d5 = (d5_close - d6_close) / d6_close * 100.0

        # 单日振幅 = (高 - 低) / 前收 * 100（输出时不带 %）
        # D-5 振幅：前收 = D-6 收盘 = closes[i-6]
        amp_d5 = (highs[i - 5] - lows[i - 5]) / d6_close * 100.0
        # D-0 振幅：前收 = D-1 收盘 = closes[i-1]
        amp_d0 = (highs[i] - lows[i]) / d1_close * 100.0

        # D-4~D-1 区间：[i-4, i-1]，区间前一日 = D-5 = closes[i-5]
        rng_highs = highs[i - 4 : i]   # D-4, D-3, D-2, D-1 四日的最高价（i-4..i-1）
        rng_lows = lows[i - 4 : i]     # D-4, D-3, D-2, D-1 四日的最低价
        range_amp = (rng_highs.max() - rng_lows.min()) / d5_close * 100.0
        # 区间涨幅：(末收 - 基收)/基收；末收 = D-1 收盘 = closes[i-1]，基收 = D-5 收盘 = closes[i-5]
        range_gain = (d1_close - d5_close) / d5_close * 100.0
        # D-4~D-1 区间内单日最大振幅：逐日 (高-低)/前收 取最大
        #   D-4 前收 = D-5 收 = closes[i-5]；D-3 前收 = D-4 收 = closes[i-4]
        #   D-2 前收 = D-3 收 = closes[i-3]；D-1 前收 = D-2 收 = closes[i-2]
        ...
        range_max_amp = max(amp_d4, amp_d3, amp_d2, amp_d1)

        # D-0/D-5 成交额百分比 = D-0成交额 / D-5成交额 * 100
        amt_d5 = amounts[i - 5]
        if amt_d5 > 0:
            amt_pct_d0_d5 = amounts[i] / amt_d5 * 100.0
        else:
            amt_pct_d0_d5 = 0.0
        ...
        rows.append({
            ...
            # 单日涨幅/振幅：数值不带 %（如 12.6）
            "D-5涨幅": round(float(gain_d5), 2),
            "D-5振幅": round(float(amp_d5), 2),
            # 区间指标：数值带 %（如 14.52%）
            "D-4~D-1区间涨幅": f"{round(float(range_gain), 2)}%",
            "D-4~D-1区间振幅": f"{round(float(range_amp), 2)}%",
            "D-4~D-1区间最大振幅": f"{round(float(range_max_amp), 2)}%",
            # 单日振幅：数值不带 %（如 12.6）
            "D-0振幅": round(float(amp_d0), 2),
            # 成交额百分比：数值带 %（如 240.14%）
            "D-0/D-5成交额百分比": f"{round(float(amt_pct_d0_d5), 2)}%",
