# -*- coding: utf-8 -*-
"""
规则0918A 筛选 —— 20cm（科创板 688 + 创业板 300/301）
...
时间线（D-0 为基准日 i，向前回看历史）：
  ...
  D-1 (i-1) : 涨停（严格封死 ==）；成交额 20 日最大；最高价 20 日最高
  D-0 (i)   : 非涨停；K 线为阳（收盘 > 开盘）—— 基准日（无成交额/股价最高条件）

形态：D-1 放量封板（成交额最大、股价最高），D-2~D-6 连续缩量非涨停，D-0 未涨停收阳线（基准日）。

输出（16 列）：
  D-0 日期
  D-0 单日涨幅、振幅（数值不带 %，前收 D-1 收）；
  最低价、最高价相对前收（D-1 收）的涨跌幅（数值不带 %，2 位小数，带正负号）
  D-1 单日振幅（数值不带 %，前收 D-2 收）
  D-0/D-1 成交额百分比（数值带 %）
...
RULE_NAME = "0918A"
OUTPUT_DIR = Path(r"D:\09work\0918A_backtest")

OUTPUT_COLUMNS = [
    "股票代码", "股票名称", "D-0日期",
    "D-0涨幅", "D-0振幅", "D-0最低价", "D-0最高价",
    "D-1振幅",
    "D-0/D-1成交额百分比",
...
def screen_one(code, day_file, start_int, end_int):
    ...
    for i in idxs:
        ...
        # D-0 最低价/最高价相对 D-1 收盘的涨跌幅（不带 %，带正负号，2 位小数）
        low_rel_d0 = (lows[i] - d1_close) / d1_close * 100.0
        high_rel_d0 = (highs[i] - d1_close) / d1_close * 100.0
        ...
        rows.append({
            ...
            "D-0最低价": f"{low_rel_d0:+.2f}",
            "D-0最高价": f"{high_rel_d0:+.2f}",
...
def main():
    parser = argparse.ArgumentParser(description=f"规则{RULE_NAME} 形态筛选（20cm）")
