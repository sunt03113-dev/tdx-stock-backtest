# -*- coding: utf-8 -*-
"""
规则0915C 筛选 —— 20cm（科创板 688 + 创业板 300/301）
...
形态：D-6 放量峰 → D-5 缩量 → D-4 再度放量且股价最高（双峰中间夹一日缩量），
      D-3/D-2/D-1 连续三日缩量，D-0 放量封板。

输出（16 列）：
  D-0 日期
  D-6 单日振幅 / D-4 单日振幅（数值不带 %，前收分别为 D-7 收、D-5 收）
  D-6~D-4 区间涨幅、区间振幅（数值带 %；基收 D-7 收，末收 D-4 收）
...
RULE_NAME = "0915C"
OUTPUT_DIR = Path(r"D:\09work\0915C_backtest")

OUTPUT_COLUMNS = [
    "股票代码", "股票名称", "D-0日期",
    "D-6振幅", "D-4振幅",
    "D-6~D-4区间涨幅", "D-6~D-4区间振幅",
...
def screen_one(code, day_file, start_int, end_int):
    ...
    for i in idxs:
        ...
        #   D-6~D-4 区间基收 = D-7 收 closes[i-7]；末收 = D-4 收 closes[i-4]
        ...
        d1_close = closes[i - 1]
        d0_close = closes[i]
        if d7_close <= 0 or d5_close <= 0 or d4_close <= 0 or d1_close <= 0 or d0_close <= 0:
            continue
        ...
        # D-6~D-4 区间（i-6, i-5, i-4；slice[i-6:i-3]）
        r64_highs = highs[i - 6: i - 3]
        r64_lows = lows[i - 6: i - 3]
        range64_amp = (r64_highs.max() - r64_lows.min()) / d7_close * 100.0
        range64_gain = (d4_close - d7_close) / d7_close * 100.0
        ...
        rows.append({
            ...
            "D-6~D-4区间涨幅": f"{round(float(range64_gain), 2)}%",
            "D-6~D-4区间振幅": f"{round(float(range64_amp), 2)}%",
