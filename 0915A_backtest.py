# -*- coding: utf-8 -*-
"""
规则0915A 筛选 —— 20cm（科创板 688 + 创业板 300/301）
...
形态：D-3/D-2 连续两日成交额峰值（D-2 同时股价最高），D-4/D-5 缩量，D-1 缩量，D-0 放量封板。

输出（16 列）：
  D-0 日期
  D-3 单日振幅 / D-2 单日振幅（数值不带 %，前收分别为 D-4 收、D-3 收）
  D-3~D-2 区间涨幅、区间振幅（数值带 %；基收 D-4 收，末收 D-2 收）
...
RULE_NAME = "0915A"
OUTPUT_DIR = Path(r"D:\09work\0915A_backtest")

OUTPUT_COLUMNS = [
    "股票代码", "股票名称", "D-0日期",
    "D-3振幅", "D-2振幅",
    "D-3~D-2区间涨幅", "D-3~D-2区间振幅",
    "D-1涨幅", "D-1振幅",
    "D-0振幅",
    "D-0/D-2成交额百分比",
...
def screen_one(code, day_file, start_int, end_int):
    ...
    for i in idxs:
        ...
        #   D-3~D-2 区间基收 = D-4 收 closes[i-4]；末收 = D-2 收 closes[i-2]
        ...
        # D-3~D-2 区间（i-3, i-2；slice[i-3:i-1]）
        r32_highs = highs[i - 3: i - 1]
        r32_lows = lows[i - 3: i - 1]
        range32_amp = (r32_highs.max() - r32_lows.min()) / d4_close * 100.0
        range32_gain = (d2_close - d4_close) / d4_close * 100.0
        ...
        rows.append({
            ...
            "D-3~D-2区间涨幅": f"{round(float(range32_gain), 2)}%",
            "D-3~D-2区间振幅": f"{round(float(range32_amp), 2)}%",
