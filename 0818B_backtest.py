# -*- coding: utf-8 -*-
"""
规则0818B 筛选 —— 20cm（科创板 688 + 创业板 300/301）
...
输出：
  1. D-0 日期
  2. D-3 单日振幅（数值不带 %，如 12.6）
  3. D-1 单日振幅（数值不带 %）
  4. D-3~D-1 区间涨幅（数值带 %，如 14.52%）
  5. D-3~D-1 区间振幅（数值带 %）
  6. D-0 单日振幅（数值不带 %）
  7. D-0/D-1 成交额百分比（数值带 %）
  8. T+0~T+6 价格走势（基准价 = D-0 收盘价）

注意：单日涨幅/振幅数值不带 %；区间涨幅/振幅数值带 %。
...
RULE_NAME = "0818B"
OUTPUT_DIR = Path(r"D:\09work\0818B_backtest")
...
OUTPUT_COLUMNS = [
    "股票代码", "股票名称", "D-0日期",
    # 单日涨幅/振幅（数值不带 %）
    "D-3振幅", "D-2涨幅", "D-2振幅", "D-1振幅", "D-0振幅",
    # 区间指标（数值带 %）
    "D-3~D-1区间涨幅", "D-3~D-1区间振幅", "D-0/D-1成交额百分比",
...
def screen_one(code, day_file, start_int, end_int):
    ...
    # 有效 i 范围：i-5>=19（D-5 的 20 日窗口起点），即 i>=24
    ...
    for i in idxs:
        ...
        # D-3~D-1 区间：[i-3, i-1]，区间前一日 = D-4 = closes[i-4]
        rng_highs = highs[i - 3 : i]   # D-3, D-2, D-1 三日的最高价
        rng_lows = lows[i - 3 : i]     # D-3, D-2, D-1 三日的最低价
        range_amp = (rng_highs.max() - rng_lows.min()) / d4_close * 100.0
        range_gain = (d1_close - d4_close) / d4_close * 100.0  # 区间涨幅：(末收 - 基收)/基收
