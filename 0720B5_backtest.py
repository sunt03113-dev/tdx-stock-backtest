# -*- coding: utf-8 -*-
"""
0720B5 筛选规则 —— tdx-stock-backtest skill 扩展
规则：
  T1-T5 连续5日，T2-T5 高低点非递降，T3+T4 双 10cm 涨停
  T6 不涨停
  Tend = T5+2~T5+14 首个涨停且非20日最强且≤T4高（无压制下搜索）
  后置过滤：T6~Tend-1 每日最高价≤T5最高价，每日成交额≤T5成交额
用法:
  python 0720B5_backtest.py
  python 0720B5_backtest.py --update
  python 0720B5_backtest.py --start 2020-01-01 --end 2025-12-31
"""
import sys
...
OUTPUT_FILE = OUTPUT_DIR / "0720B5.xlsx"
...
def screen_one(code, path, start_dt, end_dt):
    ...
    for t1 in range(1, len(df) - 5):
        ...
        # T3+T4 双涨停，T1/T2/T5 不涨停
        if limits[t1] or limits[t2] or not limits[t3] or not limits[t4] or limits[t5]:
            continue
        # T6 不涨停
        if limits[t6]:
            continue
        ...
        t5_high = df.iloc[t5]["最高价"]
        t5_amt = df.iloc[t5]["成交额(元)"]
        t4_high = df.iloc[t4]["最高价"]

        # Tend: T5+2~T5+14，首个涨停+非20日最强+≤T4高（无间隔压制）
        tend = None
        for idx in range(t5 + 2, min(t5 + 15, len(df))):
            if not limits[idx] or idx < 20:
                continue

            # Tend 最高价 ≤ T4 最高价
            if df.iloc[idx]["最高价"] > t4_high:
                continue
        ...
        # 后置过滤：T6~Tend-1 每日最高价≤T5最高价，成交额≤T5成交额
        interval_ok = True
        for s in range(t6, tend):
            if df.iloc[s]["最高价"] > t5_high or df.iloc[s]["成交额(元)"] > t5_amt:
                interval_ok = False
                break
        if not interval_ok:
            continue
...
def run_0720b5(start_date=DEFAULT_START, end_date=DEFAULT_END, do_update=False):
    ...
    print("\n===== 阶段1：0720B5规则筛选(T3+T4双涨停,T5压制,Tend非最强且≤T4高) =====")
    ...
    results = []
    for f in tqdm(main_files, desc="  0720B5筛选"):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="0720B5 涨停筛选规则")
    parser.add_argument("--update", action="store_true")
    parser.add_argument("--start", type=str, default=DEFAULT_START)
    parser.add_argument("--end", type=str, default=DEFAULT_END)
    args = parser.parse_args()
    run_0720b5(start_date=args.start, end_date=args.end, do_update=args.update)
