# -*- coding: utf-8 -*-
"""
0716A2 筛选规则 v2 —— tdx-stock-backtest skill 扩展
规则：T1-T5连续5日，T2-T5高低点非递降，仅T4 10cm涨停，T6不涨停
      Tend = T5+2~T5+11首个涨停但最高价非20日最高且成交额非20日最大
用法:
  python 0716A2_backtest.py
  python 0716A2_backtest.py --update
  python 0716A2_backtest.py --start 2020-01-01 --end 2025-12-31
"""
import sys
...
OUTPUT_FILE = OUTPUT_DIR / "0716A2.xlsx"
...
def screen_one(code, path, start_dt, end_dt):
    ...
    for t1 in range(1, len(df) - 5):
        ...
        # Tend: T5+2~T5+11，首个满足：涨停 + 最高价非20日最高 + 成交额非20日最大
        tend = None
...
def run_0716a2(start_date=DEFAULT_START, end_date=DEFAULT_END, do_update=False):
    ...
    results = []
    for f in tqdm(main_files, desc="  0716A2筛选"):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="0716A2 涨停筛选规则 v2")
    parser.add_argument("--update", action="store_true")
    parser.add_argument("--start", type=str, default=DEFAULT_START)
    parser.add_argument("--end", type=str, default=DEFAULT_END)
    args = parser.parse_args()
    run_0716a2(start_date=args.start, end_date=args.end, do_update=args.update)
