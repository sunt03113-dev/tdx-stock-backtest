# -*- coding: utf-8 -*-
"""
0719B4 筛选规则 —— tdx-stock-backtest skill 扩展
规则：
  T1-T5 连续5日，T2-T5 高低点非递降，T3+T4 双 10cm 涨停
  T6 不涨停
  间隔压制：T7~Tend-1 每日最高价 ≤ T6最高价，每日成交额 ≤ T6成交额
  Tend = T5+2~T5+14 首个涨停且非20日最强且最高价≤T6最高价
用法:
  python 0719B4_backtest.py
  python 0719B4_backtest.py --update
  python 0719B4_backtest.py --start 2020-01-01 --end 2025-12-31
"""
import sys
...
OUTPUT_FILE = OUTPUT_DIR / "0719B4.xlsx"
...
def run_0719b4(start_date=DEFAULT_START, end_date=DEFAULT_END, do_update=False):
    ...
    print("\n===== 阶段1：0719B4规则筛选(T3+T4双涨停,间隔压制,Tend非最强且≤T6高) =====")
    ...
    results = []
    for f in tqdm(main_files, desc="  0719B4筛选"):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="0719B4 涨停筛选规则")
    parser.add_argument("--update", action="store_true")
    parser.add_argument("--start", type=str, default=DEFAULT_START)
    parser.add_argument("--end", type=str, default=DEFAULT_END)
    args = parser.parse_args()
    run_0719b4(start_date=args.start, end_date=args.end, do_update=args.update)
