# -*- coding: utf-8 -*-
"""
规则0911 筛选 —— 20cm（科创板 688 + 创业板 300/301）
...
RULE_NAME = "0911"
OUTPUT_DIR = Path(r"D:\09work\0911_backtest")
...
def screen_one(code, day_file, start_int, end_int):
    ...
    # D-4(i-4): 非涨停 + 成交额 20 日最大 + 最高价 20 日最高
    ...
    # 有效 i：i-6>=19 → i>=25；T+0~T+6 不足由 generate_t_fields 返回 N/A
    cand[:25] = False
    ...
    for i in idxs:
        ...
        d1_close = closes[i - 1]
        d0_close = closes[i]
        if d5_close <= 0 or d4_close <= 0 or d1_close <= 0 or d0_close <= 0:
            continue

        # D-4 单日涨幅/振幅（数值不带 %）
        gain_d4 = (d4_close - d5_close) / d5_close * 100.0
        amp_d4 = (highs[i - 4] - lows[i - 4]) / d5_close * 100.0

        # D-3~D-1 区间（三日：i-3, i-2, i-1；slice[i-3:i]）
        rng_highs = highs[i - 3 : i]
        rng_lows = lows[i - 3 : i]
        range_amp = (rng_highs.max() - rng_lows.min()) / d4_close * 100.0
        range_gain = (d1_close - d4_close) / d4_close * 100.0
        ...
        rows.append({
            ...
            # 单日涨幅/振幅：数值不带 %
            "D-4涨幅": round(float(gain_d4), 2),
            "D-4振幅": round(float(amp_d4), 2),
            # 区间指标：数值带 %
            "D-3~D-1区间涨幅": f"{round(float(range_gain), 2)}%",
            "D-3~D-1区间振幅": f"{round(float(range_amp), 2)}%",
            # 单日振幅：数值不带 %
