# -*- coding: utf-8 -*-
"""
规则0818A 筛选 —— 20cm（科创板 688 + 创业板 300/301）
...
输出：
  1. D-0 日期
  2. D-2 单日振幅（数值不带 %，如 8.84）
  3. D-1 单日振幅（数值不带 %）
  4. D-2/D-1 区间涨幅（数值带 %，如 12.03%）
  5. D-2/D-1 区间振幅（数值带 %）
  6. D-0 单日振幅（数值不带 %）
...
RULE_NAME = "0818A"
OUTPUT_DIR = Path(r"D:\09work\0818A_backtest")
...
OUTPUT_COLUMNS = [
    ...
    "D-2振幅", "D-1振幅", "D-0振幅",
    # 区间指标（数值带 %）
    "D-2/D-1区间涨幅", "D-2/D-1区间振幅", "D-0/D-1成交额百分比",
...
def screen_one(code, day_file, start_int, end_int):
    ...
    for i in idxs:
        ...
        # D-2/D-1 区间：[i-2, i-1]，区间前一日 = D-3 = closes[i-3]
        rng_highs = highs[i - 2 : i]   # D-2, D-1 两日的最高价
        rng_lows = lows[i - 2 : i]     # D-2, D-1 两日的最低价
        range_amp = (rng_highs.max() - rng_lows.min()) / d3_close * 100.0
        range_gain = (d1_close - d3_close) / d3_close * 100.0  # 区间涨幅：(末收 - 基收)/基收
