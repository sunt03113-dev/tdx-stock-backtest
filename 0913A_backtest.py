# -*- coding: utf-8 -*-
"""
规则0913A 筛选 —— 20cm（科创板 688 + 创业板 300/301）
...
形态：D-4/D-3 连续两日成交额峰值（D-3 同时股价最高），前后各两日缩量，D-0 放量封板。

输出（16 列）：
  D-0 日期
  D-4 单日振幅 / D-3 单日振幅（数值不带 %，前收分别为 D-5 收、D-4 收）
  D-4~D-3 区间涨幅、区间振幅（数值带 %；基收 D-5 收，末收 D-3 收）
...
RULE_NAME = "0913A"
OUTPUT_DIR = Path(r"D:\09work\0913A_backtest")
...
def screen_one(code, day_file, start_int, end_int):
    ...
    for i in idxs:
        ...
        #   D-4~D-3 区间基收 = D-5 收 closes[i-5]；末收 = D-3 收 closes[i-3]
        ...
        d1_close = closes[i - 1]
        d0_close = closes[i]
        if d5_close <= 0 or d4_close <= 0 or d3_close <= 0 or d1_close <= 0 or d0_close <= 0:
            continue
        ...
        # D-4~D-3 区间（i-4, i-3；slice[i-4:i-2]）
        r43_highs = highs[i - 4: i - 2]
        r43_lows = lows[i - 4: i - 2]
        range43_amp = (r43_highs.max() - r43_lows.min()) / d5_close * 100.0
        range43_gain = (d3_close - d5_close) / d5_close * 100.0
        ...
        rows.append({
            ...
            "D-4~D-3区间涨幅": f"{round(float(range43_gain), 2)}%",
            "D-4~D-3区间振幅": f"{round(float(range43_amp), 2)}%",
