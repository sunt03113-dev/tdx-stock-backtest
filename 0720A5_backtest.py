# -*- coding: utf-8 -*-
"""
0720A5 筛选规则 —— tdx-stock-backtest skill 扩展
规则：
...
OUTPUT_FILE = OUTPUT_DIR / "0720A5.xlsx"
...
def screen_one(code, path, start_dt, end_dt):
    ...
    for t1 in range(1, len(df) - 5):
        ...
        # Tend: T5+2~T5+11，首个涨停且非20日最强（无间隔压制）
        tend = None
...
def run_0718a3(start_date=DEFAULT_START, end_date=DEFAULT_END, do_update=False):
    ...
    print("\n===== 阶段1：0720A5规则筛选(T4涨停,间隔压制,Tend非最强) =====")
    ...
    results = []
    for f in tqdm(main_files, desc="  0720A5筛选"):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="0720A5 涨停筛选规则")
