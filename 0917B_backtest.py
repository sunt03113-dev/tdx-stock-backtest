# -*- coding: utf-8 -*-
"""
规则0917B 筛选 —— 20cm（科创板 688 + 创业板 300/301）

时间线（D-0 为基准日 i，向前回看历史）：
  D-5 (i-5) : 非涨停；成交额非 20 日最大
  D-4 (i-4) : 非涨停；成交额非 20 日最大
  D-3 (i-3) : 非涨停；成交额为 20 日最大
  D-2 (i-2) : 非涨停；成交额为 20 日最大（无股价最高条件）
  D-1 (i-1) : 非涨停；成交额为 20 日最大；最高价为 20 日最高
  D-0 (i)   : 涨停（严格封死 ==）；成交额 20 日最大；最高价 20 日最高（基准日）

形态：D-3/D-2/D-1 三日成交额连续峰值（D-1 同时股价最高），D-4/D-5 缩量，D-0 放量封板。

输出（16 列）：
  D-0 日期
  D-3 单日振幅 / D-2 单日振幅 / D-1 单日振幅（数值不带 %，前收分别为 D-4 收、D-3 收、D-2 收）
...
RULE_NAME = "0917B"
OUTPUT_DIR = Path(r"D:\09work\0917B_backtest")
...
def screen_one(code, day_file, start_int, end_int):
    ...
    for i in idxs:
        ...
        d1_close = closes[i - 1]
        d0_close = closes[i]
        if d4_close <= 0 or d3_close <= 0 or d2_close <= 0 or d1_close <= 0 or d0_close <= 0:
            continue

        # D-3 / D-2 / D-1 单日振幅（数值不带 %）
        ...
        # D-0/D-1 成交额百分比
        amt_d1 = amounts[i - 1]
        amt_pct = (amounts[i] / amt_d1 * 100.0) if amt_d1 > 0 else 0.0
